...
public class MyFrame extends JFrame {
    ...
    private void initUI() {
        ...
        setLayout(new BorderLayout(10, 10)); // менеджер компоновки компас+центр
        setSize(800, 400);
        setLocationRelativeTo(null);
        
        // панель информации
        // менеджер компоновки который размещает компоненты в прямоугольной сетке с ячейками одинакового размера
        // GridLayout(строки, столбцы, горизонтальный отступ, вертикальный отступ)
        ...
        // юзер нажимает кнопку -> генерируется событие ActionEvent -> выполняется лямбда-выражение
        ...
        // JList потом заполню через updateTrainList()
        trainList = new JList<>(new String[60]); // визуальный список
        trainList.setFont(new Font("Monospaced", Font.PLAIN, 12));
        JScrollPane scrollPane = new JScrollPane(trainList); // панель прокрутки
    ...
    private int WagonDiolog(String title) {
        ...
        String railcarType = (String) JOptionPane.showInputDialog(
            this,
            "Выберите тип вагона:",        // сообщ юзеру
            title,              // заголовок окна
            JOptionPane.QUESTION_MESSAGE,  // иконка с "?"
            null,
            Railcar.vagon_types,           // варианты выбора
            Railcar.vagon_types[0]         // дефолтный вариант
        );
        
        // если юзер закрывает окно не закончив добавление вагона то railcarType = null
        ...
        // смотрим если тип вагона == тип груза по индексам 
    ...
    private void editSelectedWagon() {
        int selectedIndex = trainList.getSelectedIndex(); // получаем индекс выбранного элемента в списке
        
        // если выгон не выбран
        if (selectedIndex == -1) {
            JOptionPane.showMessageDialog(this, 
    ...
    private void addWagon(int type) {
        // проверяем не превышен ли лимит вагонов
    ...
    private void editWagon(int index, int newType) {
        // новый экземпляр для измененного вагона
    ...
    private void updateTrainList() {
        ...
        trainList.setListData(wagons); // заменяет все данные в JList новым массивом строк
    ...
    private void clearTrain() {
        ...
        if (choice == JOptionPane.YES_OPTION) {
            ...
            updateTrainList(); // теперь ничего нет в визуальном списке
        }
    }
}